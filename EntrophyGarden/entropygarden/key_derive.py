...
def _parse_path(path: str) -> List[Tuple[int, bool]]:
    """Parse BIP32 like path m/44/0/0 into a list of (index, hardened)"""
...
def derive_child(parent: bytes, path: str) -> bytes:
    """Derive a child key along a BIP32 hardened path """
...
def compute_checksum(key: bytes) -> str:
    """Return a first 8 hex character of sha 3-256 keys"""
    return hashlib.sha3_256(key).hexdigest()[:8]


def key_fingerprint(key: bytes) -> str:
    """Return a colon seperated hex fingerprint of the key"""
