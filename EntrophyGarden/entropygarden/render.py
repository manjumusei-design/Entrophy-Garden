...
as tall as wide so we account for that in the aspect ratio"""
    ...
    for gy in range(effective_h):
        line_chars = []  # Use list for O(1) append instead of the prev string concat 
        for gx in range(term_w):
            ...
            if count > 0:
                ...
                line_chars.append(map_to_glyph(255 - lum))
            else:
                line_chars.append(" ")
        lines.append(''.join(line_chars))  # Join once at the end for (O(n) instead of O(n^2))
    return lines[:term_h]


def _tile_data(data: bytes, total_size: int) -> bytes:
    if len(data) == 0:
        raise ValueError("Cannot tile empty data")
    return (data * ((total_size // len(data)) + 1))[:total_size]


def render_raw_bytes(data: bytes, width: int, height: int) -> List[str]:
    lines = []
    tiles = _tile_data(data, width * height)
    for row in range(height):
        line_chars = []
        for col in range(width):
            val = tiles[row * width + col]
            line_chars.append(map_to_glyph(val))
...
def render_heatmap(data: bytes, width: int, height: int) -> List[str]:
    lines = []
    tiled = _tile_data(data, width * height)
    for row in range(height):
        line_chars = []
        for col in range(width):
            val = tiled[row * width + col]
            ci = min(len(HEAT_COLORS) - 1, val * len(HEAT_COLORS) // 256)
            line_chars.append(f"{HEAT_COLORS[ci]}█{RESET}")
        lines.append(''.join(line_chars))
    return lines[:height]
