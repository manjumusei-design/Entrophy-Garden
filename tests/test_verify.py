...
def test_compute_response_matches_challenge():
    """Computed response should match the verifier expectation."""
    key = b"test key"
    chal = verify.generate_challenge(key)
    response = verify.compute_response(key, chal)
    assert response == chal["expected"]
    assert verify.verify_response(key, chal, response) is True
