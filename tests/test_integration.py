...
def test_ed25519_from_image_to_ssh(tmp_path):
    ...
    ssh_pub = ssh_format.to_ssh_public_key(sk.public_key, "test@entropygarden")
...
def test_read_ed25519_generated_keypair_files(tmp_path):
    """Generated Ed25519 key files should be readable by CLI workflows."""
    from entropygarden import ed25519, key_export
    from entropygarden.cli import _read_key_file

    seed = b"m" * 32
    priv_path = str(tmp_path / "ed25519_priv.pem")
    pub_path = str(tmp_path / "ed25519_pub.pub")
    key_export.write_ed25519_keypair(seed, priv_path, pub_path, {"created_at": "now"})

    read_priv = _read_key_file(priv_path)
    read_pub = _read_key_file(pub_path)

    assert read_priv == seed
    assert read_pub == ed25519.Ed25519SigningKey(seed).public_key


def test_read_x25519_generated_private_key_file(tmp_path):
    """Generated X25519 private keys should be reusable."""
    from entropygarden import key_export
    from entropygarden.cli import _read_key_file

    seed = b"n" * 32
    priv_path = str(tmp_path / "x25519_priv.pem")
    pub_path = str(tmp_path / "x25519_pub.pub")
    key_export.write_x25519_keypair(seed, priv_path, pub_path, {"created_at": "now"})

    assert _read_key_file(priv_path) == seed


def test_export_roundtrip_uses_decoded_key_material(tmp_path):
    """Exporting an existing PEM should export the decoded key, not just the wrapper bytes."""
    from entropygarden import key_export
    from entropygarden.cli import _read_key_file
    ...
    key_export.write_key(key, str(source), "pem", {"created_at": "now"})
    key_export.write_key(_read_key_file(str(source)), str(exported), "json", {"created_at": "now"})

    assert _read_key_file(str(exported)) == key
